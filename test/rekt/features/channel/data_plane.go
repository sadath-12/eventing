...
func DataPlaneChannel(channelName string) *feature.Feature {
	...
	f.Stable("Input").
		...
		Must("It MUST support both Binary Content Mode and Structured Content Mode of the HTTP Protocol Binding for CloudEvents.", func(ctx context.Context, t feature.T) {
			channelAcceptsBinaryContentMode(ctx, t)
			channelAcceptsStructuredContentMode(ctx, t)
		}).
		May("When dispatching the event, the channel MAY use a different HTTP Message mode of the one used by the event.", func(ctx context.Context, t feature.T) {
			c := getChannelable(ctx, t)
			exposedMode := c.Status.AddressStatus.Address.URL.Scheme
			newMode := exposedMode
			for _, sub := range c.Spec.SubscribableSpec.Subscribers {
				mode := sub.SubscriberURI.Scheme
				if exposedMode != mode {
					newMode = mode
				}
			}

			if newMode == exposedMode {
				t.Fatalf("expected channel to use a different HTTP Message mode than the one exposed, found: %s", exposedMode)
			}
		}).
		// For example, It MAY receive an event in Structured Content Mode and dispatch in Binary Content Mode.
		May("The HTTP(S) endpoint MAY be on any port, not just the standard 80 and 443.", func(ctx context.Context, t feature.T) {
			c := getChannelable(ctx, t)
			cCopy := c.DeepCopy()
			...
			port := addr.URL().Port()
			if port != "80" && port != "443" {
				c.Status.AddressStatus.Address.URL.Host = host + ":8080"
				patchChannelable(ctx, t, cCopy, c)
			}
		}).
		May("Channels MAY expose other, non-HTTP endpoints in addition to HTTP at their discretion.", func(ctx context.Context, t feature.T) {
			b := getChannelable(ctx, t)
			HTTPAS := 0
			for _, a := range b.Status.AddressStatus.Addresses {
				if a.URL.Scheme == "http" || a.URL.Scheme == "https" {
					HTTPAS++
				}
			}
			if len(b.Status.AddressStatus.Addresses) == HTTPAS {
				t.Fatal("expected channel to expose non-HTTP endpoints but found only HTTP(S)")
			}
		})
...
func channelAcceptsBinaryContentMode(ctx context.Context, t feature.T) {
	channelName := state.GetStringOrFail(ctx, t, "channelName")
